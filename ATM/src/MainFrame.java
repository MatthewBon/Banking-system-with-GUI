...
public class MainFrame extends JFrame {
    ...
    final static String SHOWUSERSPANEL = "Card with admin panel specified to sort users list";

    private JLabel lLogin, lPassword;
    private JTextField login, tfRegName, tfRegSurname, tfpayment, tfpayoff, tfAdmin, tfSearchAccount;
    private JPasswordField password, pfRegPassword;
    private JButton bLog, bReg, bAccReg, paymentOButton,bAdmin,adminLoginButton, checkBallance, paymentButton, payoffButton,backNButton, paymentJButton, backButton, backToLoginButton,
                    bTransactionButton, bUsersSortedByMoneyButton, bUsersSortedBySurnameButton, bGoToUsersSectionButton;
    ...
    private sortingPanel sort = new sortingPanel();
    ...
    public void addComponentToPane(Container pane) {
        ...
        adPanel.add(new JLabel("Wyszukuj historię operacji danego konta: "));
        tfSearchAccount = new JTextField(20);
        adPanel.add(tfSearchAccount);
        bTransactionButton = new JButton("Pokaż historię");
        adPanel.add(bTransactionButton);
        bTransactionButton.addActionListener(buttonListener);
        bGoToUsersSectionButton = new JButton("Przejdź do sekcji przeglądania list urzytkowników");
        adPanel.add(bGoToUsersSectionButton);
        bGoToUsersSectionButton.addActionListener(buttonListener);

        JPanel showUsersPanel = new JPanel();
        bUsersSortedByMoneyButton = new JButton("Wyświetl listę użytkowników banku posortowaną po ilości pieniędzy");
        showUsersPanel.add(bUsersSortedByMoneyButton);
        bUsersSortedByMoneyButton.addActionListener(buttonListener);
        bUsersSortedBySurnameButton = new JButton("Wyświetl listę użytkowników banku posortowaną po nazwisku");
        showUsersPanel.add(bUsersSortedBySurnameButton);
        bUsersSortedBySurnameButton.addActionListener(buttonListener);
        ...
        cards.add(showUsersPanel, SHOWUSERSPANEL);
    ...
    class ButtonListener implements ActionListener{
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            }else if(clickedButton == bAdmin) {
                CardLayout c = (CardLayout) (cards.getLayout());
                c.show(cards, ADMINLOGINPANEL);

            }else if(clickedButton == bGoToUsersSectionButton){
                CardLayout c = (CardLayout) (cards.getLayout());
                c.show(cards, SHOWUSERSPANEL);
            } else if(clickedButton == bTransactionButton){
                try {

                    adminPanel a = new adminPanel();
                    JOptionPane.showMessageDialog(((JButton)e.getSource()).getParent(), a.printAccountInformation(Integer.parseInt(tfSearchAccount.getText())));
                } catch (IOException ioException) {
                    ioException.printStackTrace();
                } catch (ParseException parseException) {
                    parseException.printStackTrace();
                }
            } else if(clickedButton == bUsersSortedBySurnameButton){
                try {
                    JOptionPane.showMessageDialog(((JButton)e.getSource()).getParent(), sort.sortingBySurnamePanel());
                } catch (ParseException parseException) {
                    parseException.printStackTrace();
                } catch (IOException ioException) {
                    ioException.printStackTrace();
                }
            } else if(clickedButton == bUsersSortedByMoneyButton){
                try {
                    JOptionPane.showMessageDialog(((JButton)e.getSource()).getParent(), sort.sortingByBallancePanel());
                } catch (ParseException parseException) {
                    parseException.printStackTrace();
                } catch (IOException ioException) {
                    //ioException.printStackTrace();
    ...
    class LoginAdmin extends  AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
             ...
             if(passA.equals("admin123")){
                 JOptionPane.showMessageDialog(((JButton)e.getSource()).getParent(), "Witamy w panelu Admina!");
                 CardLayout c = (CardLayout) (cards.getLayout());
                 c.show(cards, ADPANEL);
             }else{
                 JOptionPane.showMessageDialog(((JButton)e.getSource()).getParent(), "Błędne hasło!");
             }
